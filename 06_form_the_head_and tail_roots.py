...
# -----------------------------------------------------------------------------
# Step 6 -- forming the head and the tail roots.
#
...
# position, held still on frame 1 -- then closes the body as step 5 did, and
# then CONTINUES THE BASKET WEAVING SCHEME to its proper conclusion: the eight
# open ends are tucked away into the two roots.
#
# What step 5 did, and what changes here
# --------------------------------------
# Step 5 bent the free ending section of each of the eight ribbons back over
# itself: straight past the last crossing, a 180 degree turn inside the
# ribbon's own vertical plane, then the return run at TAIL_RISE above the base
# model.  That left eight open ends hanging in mid air at the top level.  A
# basket does not end like that.  The open end is TUCKED IN: it is pushed into
# the rim, the row of bends where the material turns from the lower layer to
# the upper one, and there it is woven under one bend and over the next.
#
# Four consequences follow from that, and all four are implemented here.
#
#   1) The bend is no longer a plain half circle.  It is now a quarter circle
#      up, a STRAIGHT VERTICAL SECTION, and a quarter circle over.  The
#      straight part is what makes the rim a row of parallel flat walls with a
#      parallel-sided SLOT behind them -- a half circle has no two points at
#      the same height, so it grips nothing.  Right at the wall plane the two
#      limbs of a bend are exactly TURN_STRAIGHT apart, and that gap is the
#      slot the tucked ends are threaded through.
#
#   2) The straight section has to be TWICE the height one end needs, because
#      TWO ends go through the same slot.  The four lanes of a group lie edge
#      to edge; two of them bend back at one side of the body and two at the
#      other, so at each rim two ends arrive and both weave along the same row
#      of bends, one high in the slot and one low, in opposite order.  With one
#      end's worth of height they would have to pass through one another.
#      TURN_STRAIGHT therefore goes from 1 to 2 cm and TAIL_RISE with it, from
#      5 to 6, built as 2 + 2 + 2 (quarter turn, straight wall, quarter turn).
#
#   3) The tucked end is NEVER FOLDED and never turned over.  It has to turn a
#      quarter turn in PLAN to get into the rim, because the rim runs across
#      the lanes and the end arrives along one -- and a strip lying flat cannot
#      turn inside its own plane at all without tearing, since the outer edge
#      would have to be longer than the inner one.  There are exactly two ways
#      out of that: FOLD it, which creases it and lays it over onto its other
#      face, or ROLL IT ONTO ITS EDGE and curve round while standing on it,
#      where the same turn is an ordinary bend and the surface swept is a plain
#      vertical cylinder.  The roll is what a real end does, and what is done
#      here.  It rolls up for the corner and lies flat again straight after --
#      because inside the rim it has to RISE AND DIP, low at one bend and high
#      at the next, and rising is the one thing an on-edge strip cannot do.
#      Each orientation buys exactly one of the two movements:
#           lying FLAT   -> free to rise and dip, cannot turn in plan
#           on its EDGE  -> free to turn in plan, cannot rise or dip
#      so the end is flat along the return run, rolled up through the quarter
#      turn, flat again through the rim, rolled up through the turn onto the
#      root's axis, and flat out along the stem.  Seen from beside a lane it
#      therefore shows only its EDGE before a corner and only its EDGE after
#      it, and its face just while it is rolled up, in the corner itself.
#
#   4) The ends have to STOP somewhere, and the place is fixed by the weave
#      itself.  Once an end has passed the two bends named for it, it is
#      travelling ALONG the rim -- that is, along the lanes of the other
#      diagonal -- and the two ends coming out of the other diagonal's rim are
#      travelling straight at it.  Four ends meet in the corner beyond the two
#      rims.  They are not allowed to shoot past one another there: they are
#      GATHERED: each rolls round once more onto the root's own axis, and the
#      four run out along it stacked one on another.  That bundle is the stem
#      the head (or the tail) will be formed from in a later step.  Each strand
#      keeps whatever length it has left, so the bundle is ragged at the far
#      end -- which is what a bundle of woven ends really looks like, and what
#      the next step will shape.
#
# Where each end goes
# -------------------
# A ribbon turns back at its OWN outer end and then runs the whole length of
# the body at the upper level, so its open end arrives at the OPPOSITE side --
# exactly where the bends of its three parallel lane-mates stand.  That is the
# rim it tucks into.  The four folded ribbons (green, red, orange, purple) turn
# back at one side and tuck in at the other, forming the HEAD root; the four
# straight ribbons (brown, blue, gray, black) do the mirror of it and form the
# TAIL root.  The two roots are therefore on opposite sides of the body, which
# is what the two schemes below describe.
#
# The stem of the head:      The stem of the tail (the opposite side):
#   green  under gray  over  black      brown over  orange under purple
#   red    over  gray  under black      blue  under orange over  purple
#   orange over  brown under blue       gray  under green  over  red
#   purple under brown over  blue       black over  green  under red
#
# Read a line as one tucked end and the two bends it threads through, in the
# order it meets them.  The tail scheme is the head scheme inverted pair by
# pair -- the same eight pairs of ribbons meet twice, once at each root, and a
# weave alternates -- which is the check that the two roots really are the two
# ends of one continuous basket weave.
#
# Each of the two blocks above is one STEM: its four lines are the four ends
# that meet in that corner and are pulled together there.  Two of them arrive
# along one diagonal and two along the other, head on, which is why they have
# to be gathered rather than simply run out.
#
# ONE END, AND ONE MOVEMENT, AT A TIME.  The tucking-in is being built up in
# stages so that each stage can be looked at on its own, against a rim that is
# standing still.  Every ribbon except the one being worked on is left exactly
# as step 5 leaves it -- bent back and running dead straight home at TAIL_RISE.
#
# BUILT SO FAR, all of it on the BLACK ribbon and nothing else:
#
#   a) Its run home ends in a ninety degree bend DOWNWARDS, taken where the
#      loops of green and red -- the two it is to be woven through -- begin
#      their own bends.  Up to that point it runs parallel to them and level
#      with them; there they curve down and turn back, and it curves down and
#      carries straight on.  A plain bend: not turned in plan, not rolled, and
#      as flat after it as before it.  Because it has the same radius as their
#      bends, it lands the drop exactly on the plane their straight walls stand
#      in.
#
#   b) A quarter turn INSIDE that plane, from pointing down to pointing along
#      the row of bends, so that the end now runs ACROSS green and red instead
#      of alongside them.  This is the turn a strip cannot simply bend into, and
#      it is made the way a real one is: a section in the middle lifts off the
#      plane -- see roll_angle -- rather than being creased.  No fold, and the
#      ribbon is not turned over.
#
# STILL TO COME:  the end now runs along the rim IN THE SAME PLANE as green's
# and red's straight walls, so it neither passes in front of them nor behind
# them.  Which side of each it takes -- the over and under of the scheme -- is
# the next thing to build, and so is where it stops.  The scheme lines above say
# which bends each of the eight ends belongs to, and they are checked, but only
# the black one is built at all.
#
...
# =============================================================================
# CLOSING THE BODY -- the free ribbon ends bend back over themselves, and then
# tuck into the rim at the far side.
#
...
#
# The bend profile, in the ribbon's own vertical plane, measured as arc length
# from where the bend starts:
#
#       . . . . . . . . . . . . . . . .  return run, z = TAIL_RISE
#     .'
#    |     <- quarter circle over,  radius TURN_RADIUS
#    |
#    |     <- STRAIGHT WALL, height TURN_STRAIGHT.  This is the new part: the
#    |        rim needs parallel flat walls, because only between two PARALLEL
#    |        limbs is there a slot of constant height for an end to be
#     '.      threaded through.  A plain half circle offers no such surface.
#       . . . . . . . . . . . . . . . .  base model, z = 0
#             <- quarter circle up, radius TURN_RADIUS
#
# TAIL_RISE = 2 * TURN_RADIUS + TURN_STRAIGHT, so the three pieces have to add
# up.  They are sized by THE SLOT -- the gap the bend leaves between its two
# limbs.  Seen along a lane the bend is a "C" lying on its back, open towards
# the middle of the weave and closed at the outer end by the straight wall, and
# the tucked ends are woven inside it, lying flat:
#
#          upper layer  . . . . . . .
#                                     `-.         <- the upper limb
#                            ~~~~~~~~~~~~~~ |     <- an end riding HIGH in the
#            the slot                       |        slot: it has gone UNDER
#                            ~~~~~~~~~~~~~~ |     <- and one riding LOW: it has
#                                     .-'   |        gone OVER
#          base layer   . . . . . . .       |
#                                           ^ the straight wall: here, and only
#                                             here, the limbs are parallel and
#                                             exactly TURN_STRAIGHT apart
#
# That is what the straight part is really for, and why its height is what it
# is.  Two ends have to go through the same slot -- see point 2 at the top of
# the file -- one riding high in it and one low, and each has to clear the
# other and the limb it is tucked against.  TURN_STRAIGHT is therefore TWICE
# what one end on its own would need.
#
# The four lanes of a group are edge to edge, so the slots of the whole row
# join into one continuous channel running along the rim.  A tucked end travels
# ALONG that channel, which is why it has to be turned a quarter turn in plan
# first.  That turn is not built yet: so far the end only bends DOWN, where
# the bends of the rim begin.
# =============================================================================
TAIL_OFFSET = 1.0                    # straight run left after the last crossing
TURN_RADIUS = 2.0                    # radius of both quarter circles
TURN_STRAIGHT = 2.0                  # the straight wall between them -- TWICE
                                     # the old 1.0, because TWO ends stand in
                                     # the slot it opens, one above the other
TAIL_RISE = 2.0 * TURN_RADIUS + TURN_STRAIGHT   # 6.0: the upper layer's height
QUARTER_ARC = 0.5 * math.pi * TURN_RADIUS
TURN_ARC = 2.0 * QUARTER_ARC + TURN_STRAIGHT    # material consumed by the turn

WALL_FOOT = TURN_RADIUS                         # z where the straight wall starts
WALL_HEAD = TURN_RADIUS + TURN_STRAIGHT         # z where it ends

# THE TWO PASS HEIGHTS.  A tucked end goes through the rim LYING FLAT, and it
# has to: inside the rim it rides low at one bend and high at the next, and a
# strip changes height by BENDING, which it can only do about an axis lying
# across its width.  Flat, that axis is horizontal and rising or dipping costs
# nothing; stood on its edge the same rise would be a bend inside its own
# plane, which no strip can make without stretching one edge past the other.
# So it is flat here, and would only stand on edge where it turns in PLAN.
#
# Inside the slot there are exactly two places for it:
#   * HIGH, up against the bend's upper limb   -> it has gone UNDER that bend
#   * LOW,  down on the bend's lower limb      -> it has gone OVER it
# so "under" is the high pass and "over" is the low one.  That is the opposite
# of what it would be out in the open, and it is why the slot is the only place
# this weave can happen.  The two ends of a rim take one place each, in
# opposite order, and THAT is what the straight wall's height has to cover: at
# the wall plane the slot is exactly TURN_STRAIGHT tall, and the two ends have
# to ride in it clear of each other and of both limbs.
#
# The end reaches into the slot far enough to be held and no further: its outer
# edge stops PASS_INSET short of the wall plane, so its centre line runs half a
# ribbon width inside that.  A little way in, the limbs have already curved
# apart by PASS_OPEN above and below, which is the slack the pass heights are
# checked against.
PASS_INSET = 0.15                    # the end's outer edge stops this far short
                                     # of the wall plane: tight against it
PASS_GRIP = 0.40                     # how far in from the slot's foot and head
                                     # the two ends ride
PASS_OPEN = math.sqrt(max(TURN_RADIUS ** 2 - (TURN_RADIUS - PASS_INSET) ** 2, 0.0))
PASS_LOW = WALL_FOOT + PASS_GRIP                # 2.4, over the lower limb
PASS_HIGH = WALL_HEAD - PASS_GRIP               # 3.6, under the upper one
if PASS_HIGH - PASS_LOW < W - PASS_OPEN:
    print("Rim warning: the straight wall is too short for two ends to pass "
          f"through it at once; TURN_STRAIGHT needs at least "
          f"{W - PASS_OPEN + 2.0 * PASS_GRIP:.2f} cm.")

# THE TURN ONTO THE RIM'S LINE.  Once the end is pointing straight down it is
# lying flat against THE WALL PLANE -- the one vertical plane that holds the
# straight sections of every bend in this rim, because the down bend has the
# same radius as they do and so lands the drop exactly on them.  The end now
# has to turn a quarter circle INSIDE that plane, from pointing down to
# pointing along the row, so that it runs across green and red instead of
# alongside them.
#
# A turn inside the ribbon's own plane is the one turn a strip cannot simply
# bend into: the outer edge of the corner is longer than the inner one.  Press a
# ribbon flat on a table, hold the far end down and swing it a quarter turn --
# both ends stay flat on the table and a section in the middle HAS TO LIFT off
# it.  That lift is what is modelled here.  Through the corner the ribbon rolls
# a quarter turn about its own length, so that its width comes out of the wall
# plane and lies along the axis the corner turns about; there the bend is an
# ordinary one and costs nothing.  It rolls back flat before and after, so the
# drop and the run along the rim both lie in the wall plane.  No crease, and the
# ribbon is not turned over.
# HOW THE WHOLE MANOEUVRE IS SIZED.  It is one gradual movement, and its three
# pieces share out the height between the two layers: the quarter circle DOWN
# eats DOWN_RADIUS of it, the straight DROP eats its own length, and the quarter
# turn onto the rim eats CROSS_RADIUS.  What is left is the height the end comes
# out at, and every one of the four is chosen for a reason.
#
# DOWN_RADIUS is as large as it can be, because the manoeuvre wants to start as
# EARLY as possible and be as gentle as possible.  The drop has to land on the
# wall plane -- the one vertical plane holding the straight walls of every bend
# in this rim -- so the bend must begin exactly DOWN_RADIUS short of it, and the
# bigger the radius the further back that is.  The limit is the raised lattice:
# the end has to stay flat and level until its last crossing up there is behind
# it, which is checked below.
#
# DROP is where the ROLL is made, and it is long enough to make it in.  Twisting
# a strip on a straight run neither gathers it nor stretches it; twisting it
# while it is also turning does both, and that is what pinches.
#
# CROSS_RADIUS is large as well, so that the turn onto the rim is a long sweep,
# and it decides where the manoeuvre ENDS: the turn moves the end CROSS_RADIUS
# along the rim, and it has to be over and done with before the end arrives at
# the first bend it must weave through.  That clearance is checked below too.
DOWN_RADIUS = 2.5                         # the quarter circle down
DROP = 1.25                               # the straight piece the roll is made on
CROSS_RADIUS = 1.25                       # the quarter turn onto the rim's line
DOWN_ARC = 0.5 * math.pi * DOWN_RADIUS
CROSS_ARC = 0.5 * math.pi * CROSS_RADIUS
ROLL_OUT = 1.0                            # arc length along the rim to unroll in
# THE HEIGHT IT COMES OUT AT is then whatever is left -- and it wants to be low,
# because two ends run along this rim, this one and the one from the lane next
# door, and they run the same way down the same line.  They take one place each:
# this one the LOWER, the other the one directly above it.
CROSS_Z = TAIL_RISE - DOWN_RADIUS - DROP - CROSS_RADIUS   # 1.0
if CROSS_Z - 0.5 * W < 0.0:
    print("Rim warning: the end comes out of its turn low enough to reach "
          "through the base weave.")

# The stem.  Beyond the rim the four ends of one root come together on a single
# line and run out along it, stacked face to face -- pulled into a bundle.  They
# are flat, so they stack in HEIGHT: four of them make a stalk one ribbon width
# wide and three pitches deep.
STEM_REACH = 1.2                     # how far beyond the outermost end of the rim
                                     # the four of them come together
STEM_PITCH = 0.3                     # face-to-face spacing inside the bundle
STEM_LENGTH = 14.0                   # how far the bundle is drawn out; a strand
                                     # that runs out of material simply stops
STEM_Z = 0.5 * (PASS_HIGH + PASS_LOW)           # 3.0, the slot's own mid height:
                                     # the ends that leave the rim low rise to it
                                     # by as much as the high ones drop
...
def ribbon_outward(name):
    """Which way along its lane this ribbon reaches farther from the core."""
    obj = bpy.data.objects[name]
    centre, axis = lanes[name]
    core_u = (WEAVE_CORE - centre).dot(axis)
    reach = [(flat(obj.matrix_world @ vertex.co) - centre).dot(axis)
             for vertex in obj.data.vertices]
    return 1.0 if (max(reach) - core_u) >= (core_u - min(reach)) else -1.0


OUTWARD = {name: ribbon_outward(name) for name in RIBBON_NAMES}

# Every ribbon gets a right-handed frame of its own: FORWARD points outward
# along its lane, SIDE across it, UP is the world up.  All the tail geometry
# below is written in this frame as (u, v, z) and converted at the last moment,
# which keeps it readable no matter which of the four diagonals the ribbon is
# on or which way round it lies.
FRAMES = {}
for _name in RIBBON_NAMES:
    _centre, _axis = lanes[_name]
    _forward = _axis * OUTWARD[_name]
    FRAMES[_name] = (_centre, _forward, UP.cross(_forward).normalized())


def frame_point(name, u, v, z):
    centre, forward, side = FRAMES[name]
    return centre + forward * u + side * v + UP * z


def frame_coords(name, point):
    """A world point as (u, v, z) in one ribbon's own lane frame."""
    centre, forward, side = FRAMES[name]
    planar = flat(point) - centre
    return planar.dot(forward), planar.dot(side), point.z


# Where each ribbon's bend starts: TAIL_OFFSET past the far edge of the
# outermost crossing it takes part in.  The ribbon crossing it there runs
# perpendicular, so that crossing ends half a ribbon width past its centre.
BEND_U = {}
INNER_U = {}          # the far edge of the crossing at the OTHER end of the lane
for _name in RIBBON_NAMES:
    _centre, _forward, _side = FRAMES[_name]
    _reach = [(lane_crossing(_name, _other) - _centre).dot(_forward)
              for _other in LANE_PARTNERS[_name]]
    BEND_U[_name] = max(_reach) + 0.5 * W + TAIL_OFFSET
    INNER_U[_name] = min(_reach) - 0.5 * W


def bend_profile(s):
    """The bend's centre line at arc length s: (du, z, tangent_u, tangent_z).

    Quarter circle up, straight wall, quarter circle over -- see the diagram
    with the constants.  du is measured outward from where the bend starts.
    """
    if s <= QUARTER_ARC:
        theta = s / TURN_RADIUS
        return (TURN_RADIUS * math.sin(theta),
                TURN_RADIUS * (1.0 - math.cos(theta)),
                math.cos(theta), math.sin(theta))
    if s <= QUARTER_ARC + TURN_STRAIGHT:
        return (TURN_RADIUS, WALL_FOOT + (s - QUARTER_ARC), 0.0, 1.0)
    theta = (s - QUARTER_ARC - TURN_STRAIGHT) / TURN_RADIUS
    return (TURN_RADIUS * math.cos(theta),
            WALL_HEAD + TURN_RADIUS * math.sin(theta),
            -math.sin(theta), math.cos(theta))


def wall_centre(name):
    """The world point at the middle of one ribbon's straight rim wall."""
    centre, forward, _side = FRAMES[name]
    return (centre + forward * (BEND_U[name] + TURN_RADIUS)
            + UP * (0.5 * (WALL_FOOT + WALL_HEAD)))


# -----------------------------------------------------------------------------
# THE TWO ROOT SCHEMES.
#
# Colours, as the two module palettes assign them:
#   module 1 (right):  brown Csik_1, blue Csik_2, green Csik_3, red Csik_4
#   module 2 (left):   gray  Csik_1, black Csik_2, orange Csik_3, purple Csik_4
# Csik_1 and Csik_2 are the straight standing threads, Csik_3 and Csik_4 the
# folded ones, so "the four folded" is green, red, orange, purple and "the four
# straight" is brown, blue, gray, black.
#
# Each line is one tucked-in open end and the two rim bends it threads through,
# named in the order it meets them.  Both bends belong to ribbons on lanes
# PARALLEL to the tucked one -- that is what a rim is, and it is checked below.
# -----------------------------------------------------------------------------
COLOUR_RIBBON = {
    "brown":  "M1_Csik_1",
    "blue":   "M1_Csik_2",
    "green":  "M1_Csik_3_hajtott",
    "red":    "M1_Csik_4_hajtott",
    "gray":   "M2_Csik_1",
    "black":  "M2_Csik_2",
    "orange": "M2_Csik_3_hajtott",
    "purple": "M2_Csik_4_hajtott",
}

HEAD_STEM_SCHEME = [
    ("green",  ("gray",  "under"), ("black",  "over")),
    ("red",    ("gray",  "over"),  ("black",  "under")),
    ("orange", ("brown", "over"),  ("blue",   "under")),
    ("purple", ("brown", "under"), ("blue",   "over")),
]

TAIL_STEM_SCHEME = [
    ("brown",  ("orange", "over"),  ("purple", "under")),
    ("blue",   ("orange", "under"), ("purple", "over")),
    ("gray",   ("green",  "under"), ("red",    "over")),
    ("black",  ("green",  "over"),  ("red",    "under")),
]

TUCK_PLAN = {}          # ribbon name -> (root, [(bend ribbon, "under"/"over")])
ROOT_MEMBERS = {"head": [], "tail": []}
for _root, _scheme in (("head", HEAD_STEM_SCHEME), ("tail", TAIL_STEM_SCHEME)):
    for _subject, _first, _second in _scheme:
        _name = COLOUR_RIBBON[_subject]
        TUCK_PLAN[_name] = (_root, [(COLOUR_RIBBON[_first[0]], _first[1]),
                                    (COLOUR_RIBBON[_second[0]], _second[1])])
        ROOT_MEMBERS[_root].append(_name)

for _name, (_root, _tucks) in TUCK_PLAN.items():
    for _bend_name, _side in _tucks:
        if abs(FRAMES[_name][1].dot(FRAMES[_bend_name][1])) < 0.5:
            print(f"Root scheme warning: {_name} and {_bend_name} are not on "
                  f"parallel lanes, so there is no rim bend to tuck through.")

# HOW FAR THE TUCKING-IN HAS BEEN BUILT.  The eight schemes above describe the
# finished basket.  It is built one end at a time and, for now, one MOVEMENT at
# a time, so that each can be looked at against a rim that is standing still.
#
# What is built so far is the FIRST MOVEMENT of one end: the black ribbon's
# ninety degree bend DOWNWARDS.  Nothing else -- no turn in plan, no roll, no
# crossing.  Every other ribbon is left exactly as step 5 leaves it: bent back
# and running dead straight home at TAIL_RISE, its end stopping in mid air.
DOWN_BENT_RIBBONS = ["M2_Csik_2"]               # the black ribbon, and it alone


def rim_wall_u(name):
    """Where the wall plane of the rim this end is heading for cuts its lane.

    The two bends it is to be woven through belong to ribbons whose lanes are
    PARALLEL to its own, and their straight walls stand at the same distance
    from the core -- that is what makes a rim a straight row rather than a
    staircase, and it is what lets one number describe the whole plane.
    """
    _root, tucks = TUCK_PLAN[name]
    walls = [frame_coords(name, wall_centre(bend_name))[0]
             for bend_name, _side in tucks]
    if max(walls) - min(walls) > 0.01:
        print(f"Rim warning: the two bends {name} is aimed at do not stand at "
              "the same distance from the core, so the rim is not a straight row.")
    return max(walls)


def rim_reach(name):
    """How far along the rim the end may turn before it is at the first bend."""
    (_u, v, _z), _second, _step = rim_marks(name)
    return abs(v) - 0.5 * W


# Two ends thread the SAME pair of bends at every root: they travel the same
# way along the same rim, in the same slot -- one high-then-low, the other
# low-then-high.  Being in opposite phase they meet midway between the two
# bends, and there both of them are crossing the slot's mid height at once.
# That crossing is the last thing left to solve before the second end of a rim
# can be built; while only one is tucked in it does not arise.
def rim_marks(name):
    """Where one end passes through the rim, in its own lane frame.

    Returns ((u, v1, z1), (u, v2, z2), step): the centre of the first and of
    the second pass, and which way along the rim the end is travelling.

    Both passes are at the SAME u -- the end runs along the rim, parallel to
    the wall plane -- and that u is fixed by the wall: the end lies flat with
    its outer edge PASS_INSET short of the wall plane, so its centre line runs
    half a ribbon width further in.  The two heights come from the scheme:
    under a bend means high in the slot, up against its upper limb; over a bend
    means low in the slot, down on its lower one.
    """
    _root, tucks = TUCK_PLAN[name]
    marks = []
    for bend_name, side in tucks:
        u, v, _z = frame_coords(name, wall_centre(bend_name))
        u += PASS_INSET + 0.5 * W
        marks.append((u, v, PASS_HIGH if side == "under" else PASS_LOW))
    return marks[0], marks[1], (1.0 if marks[1][1] >= marks[0][1] else -1.0)


def rim_exit(name):
    """The world point where an end leaves the tunnel, and the way it is going.

    The run along the tunnel reaches from one edge of the second bend's lane to
    the other and no further; past that edge the end is clear of the rim,
    because the lanes are edge to edge and the fourth lane of the row -- the
    other tucked end's own lane -- carries no bend at this side.
    """
    (_u1, _v1, _z1), (u2, v2, z2), step = rim_marks(name)
    return (frame_point(name, u2, v2 + 0.5 * W * step, z2),
            FRAMES[name][2] * step)


# -----------------------------------------------------------------------------
# THE STEM: where the four ends of one root meet.
#
# Each end leaves its rim travelling along it, which means along the lanes of
# the OTHER diagonal -- and the two ends coming out of that other diagonal's
# rim are travelling straight back at it.  Left alone they run through one
# another in the corner.  They are gathered instead: each turns once more, onto
# the root's own axis of symmetry, and they run out along it stacked one on top
# of another.
#
# The axis starts from the mean of the four wall centres and points directly
# away from the core of the weave; the bundle lies at the rim's mid height.  The
# ends are FLAT, so the bundle is a stack: four blades STEM_PITCH apart in
# height, all on the same line.  They are stacked in the order they arrive --
# the two that leave the tunnel high take the top two places -- so no strand has
# to climb past another to reach its own.
# -----------------------------------------------------------------------------
STEM_LEVEL = {}         # ribbon name -> the height its strand runs at
STEM_AXIS = {}          # ribbon name -> the direction the bundle runs
ROOT_STEM = {}          # root -> the base of the bundle
for _root in ("head", "tail"):
    _members = ROOT_MEMBERS[_root]
    _exits = {_m: rim_exit(_m)[0] for _m in _members}
    _walls = [wall_centre(_bend_name)
              for _m in _members
              for _bend_name, _side in TUCK_PLAN[_m][1]]
    _hub = sum(_walls, Vector((0.0, 0.0, 0.0))) / len(_walls)
    _axis = flat(_hub - WEAVE_CORE).normalized()
    _out = max((flat(_exits[_m]) - flat(_hub)).dot(_axis) for _m in _members)
    ROOT_STEM[_root] = flat(_hub) + _axis * (_out + STEM_REACH) + UP * STEM_Z
    _order = sorted(_members, key=lambda m: -_exits[m].z)
    for _i, _m in enumerate(_order):
        STEM_LEVEL[_m] = STEM_Z + (0.5 * (len(_order) - 1) - _i) * STEM_PITCH
        STEM_AXIS[_m] = _axis


# Which way along the rim the end has to go once it is down: towards the first
# of the two bends named for it.  Both of them lie the same way, so the sign of
# the nearer one is the sign of the whole run.
CROSS_STEP = {name: math.copysign(1.0, rim_marks(name)[0][1])
              for name in DOWN_BENT_RIBBONS}

# Where the down bend begins.  It is measured back from the WALL PLANE, not
# forward from anything, because the drop has to land on that plane: bend a
# quarter circle of radius DOWN_RADIUS and you travel exactly that far in plan,
# so the bend has to start exactly that far short of it.
WALL_U = {name: rim_wall_u(name) for name in DOWN_BENT_RIBBONS}
DOWN_U = {name: WALL_U[name] + DOWN_RADIUS for name in DOWN_BENT_RIBBONS}
for _name in DOWN_BENT_RIBBONS:
    if DOWN_U[_name] > INNER_U[_name]:
        print(f"Rim warning: {_name} would start bending down "
              f"{DOWN_U[_name] - INNER_U[_name]:.2f} cm before the last crossing "
              "of the raised lattice, which would cost that lattice a crossing; "
              "DOWN_RADIUS is too big.")
    if CROSS_RADIUS + ROLL_OUT > rim_reach(_name):
        print(f"Rim warning: {_name} is still turning, or still unrolling, when "
              "it reaches the first bend it has to weave through; "
              f"CROSS_RADIUS + ROLL_OUT must be under {rim_reach(_name):.2f} cm.")


def roll_angle(t, begin, end):
    """How far the ribbon is rolled about its own length at arc length t.

    This is the lift.  Press a ribbon flat on a table, hold both ends down and
    swing it round a quarter turn: the ends stay down and a section in the
    middle has to come up.  Rolled a quarter turn the ribbon's width points out
    of the wall plane, along the very axis the corner turns about, which is the
    one direction a strip bends in for nothing -- and at nought it lies flat in
    that plane again, which is where the drop before the corner and the run
    along the rim after it both belong.

    The shape of it matters as much as the size.  The roll is FULLY ON before
    the corner starts and stays on until it ends, so that the whole of the turn
    is made with the width square to it; a corner taken half rolled has an outer
    edge longer than its inner one, and the ribbon gathers there and pinches.
    Coming on and going off therefore happen on the STRAIGHT pieces either side
    -- the drop before, the run along the rim after -- where a twist costs the
    ribbon nothing at all.  The drop is exactly as long as the roll needs.
    """
    if t <= begin - DROP:
        return 0.0
    if t < begin:
        ...
        f = (end + ROLL_OUT - t) / ROLL_OUT
    else:
        return 0.0
    return 0.5 * math.pi * f * f * (3.0 - 2.0 * f)      # smooth on, smooth off


def down_frame(name, t):
    """Where a down-bent end is at arc length t past the start of its down bend.

    Returns (centre line point, width direction, thickness direction).  Four
    pieces, in order:

      THE QUARTER CIRCLE DOWN, radius DOWN_RADIUS -- big, and starting that far
        short of the wall plane, so that the drop it leads into lands exactly on
        the plane the straight walls of this rim stand in.
      THE DROP, straight, flat against that plane, and the length the roll takes
        to come on.
      THE QUARTER TURN ONTO THE RIM'S LINE, radius CROSS_RADIUS, taken inside
        that same plane -- from pointing down to pointing along the row of
        bends, so that the end now runs ACROSS green and red instead of
        alongside them.  This is the turn the ribbon has to roll through; see
        the note with CROSS_Z and roll_angle.
      THE RUN ALONG THE RIM, straight, at CROSS_Z -- the lower of the two places
        a rim has, the upper one being left for the end coming from the lane
        next door.

    The first two pieces bend about the ribbon's own width and cost nothing; it
    is the third that has to be rolled through, and roll_angle rolls it on over
    the drop and off again over the run along the rim.
    """
    centre, forward, side = FRAMES[name]
    step = CROSS_STEP[name]
    along = side * step                      # towards the bends it has to cross
    foot = (centre + forward * (DOWN_U[name] - DOWN_RADIUS)
            + UP * (TAIL_RISE - DOWN_RADIUS))          # bottom of the down bend
    turn_start = t - DOWN_ARC - DROP                   # arc length into the turn

    if t <= DOWN_ARC:                                  # the quarter circle down
        theta = t / DOWN_RADIUS
        origin = (centre + forward * (DOWN_U[name] - DOWN_RADIUS * math.sin(theta))
                  + UP * (TAIL_RISE - DOWN_RADIUS * (1.0 - math.cos(theta))))
        return origin, side, forward * math.sin(theta) - UP * math.cos(theta)

    if turn_start <= 0.0:                              # the straight drop
        return foot - UP * (t - DOWN_ARC), side, forward

    top = foot - UP * DROP                             # where the turn begins
    if turn_start <= CROSS_ARC:                        # the quarter turn
        psi = turn_start / CROSS_RADIUS
        hub = top + along * CROSS_RADIUS
        origin = hub - (along * math.cos(psi) + UP * math.sin(psi)) * CROSS_RADIUS
        tangent = along * math.sin(psi) - UP * math.cos(psi)
        width = (along * math.cos(psi) + UP * math.sin(psi)) * step
        return origin, width, tangent.cross(width)

    hub = top + along * CROSS_RADIUS                   # the run along the rim
    origin = hub - UP * CROSS_RADIUS + along * (turn_start - CROSS_ARC)
    width = UP * step
    return origin, width, along.cross(width)


def bend_free_tail(name):
    """Bend one ribbon's free end back, run it home, and -- for one of them --
    bend it down again at the far side.
    ...
    the ribbon is re-laid, in order, along
      * the bend: quarter circle, straight wall, quarter circle (TURN_ARC),
      * the return run at TAIL_RISE, straight back over its own outgoing half,
      * and, for the ribbons in DOWN_BENT_RIBBONS only, the route down and onto
        the rim that down_frame lays out: a quarter circle DOWN where the rim it
        is heading for begins, a short drop, a quarter turn onto the rim's own
        line, and the run along it.  For every other ribbon the return run
        simply carries on to the end of the material, exactly as step 5 leaves
        it.

    The material coordinate is the arc length throughout, so the ribbon neither
    stretches nor shrinks -- it only reaches less far in plan view, by exactly
    what the bends consume.
    """
    obj = bpy.data.objects[name]
    centre, forward, side = FRAMES[name]
    bend_u = BEND_U[name]
    down_u = DOWN_U.get(name)
    # Arc length at which the run home gives way to the down bend.  Infinite for
    # a ribbon that has no down bend, so that its run home never ends.
    down_s = math.inf if down_u is None else TURN_ARC + bend_u - down_u
    # And, measured from there, the stretch the roll covers: the quarter turn
    # onto the rim's line, with ROLL_LEAD either side of it.
    roll_on = DOWN_ARC + DROP
    roll_off = roll_on + CROSS_ARC
    ...
    for vertex, point in zip(obj.data.vertices, points):
        planar = flat(point)
        u = (planar - centre).dot(forward)
        if u <= bend_u:
            continue                                  # still the base model
        n = (planar - centre).dot(side)               # across the ribbon: kept
        b = point.z                                   # its baked weave offset
        s = u - bend_u                                # arc length past the bend
        if s <= TURN_ARC:
            # In the bend the thickness offset rides along the turning normal,
            # which is the tangent rotated a quarter turn inside the plane.
            du, z, tu, tz = bend_profile(s)
            world = (centre + forward * (bend_u + du - tz * b) + side * n
                     + Vector((0.0, 0.0, z + tu * b)))
        elif s <= down_s:
            # The return run: reverse direction, straight, at TAIL_RISE.  The
            # ribbon is upside down here, hence the flipped thickness offset.
            # For a ribbon with no down bend down_s is infinite, so this is
            # where its material runs out -- straight, as step 5 left it.
            world = (centre + forward * (bend_u - (s - TURN_ARC)) + side * n
                     + Vector((0.0, 0.0, TAIL_RISE - b)))
        else:
            # Down, round, and away along the rim.  The roll is applied to the
            # cross section only, so it turns the ribbon over its own centre
            # line without moving or lengthening it.
            origin, width, thick = down_frame(name, s - down_s)
            angle = roll_angle(s - down_s, roll_on, roll_off)
            world = (origin
                     + (width * math.cos(angle) + thick * math.sin(angle)) * n
                     + (thick * math.cos(angle) - width * math.sin(angle)) * b)
...
# The two roots, as handles the shaping stages can grab.  Each sits at the base
# of its stem -- the point where its four ends come together -- so a later step
# can take hold of the bundle itself rather than of the rim behind it.
root_empties = {}
for _root in ("head", "tail"):
    _hungarian = "Fej_gyoker" if _root == "head" else "Farok_gyoker"
    _empty = add_empty(_hungarian, body_root)
    _empty.location = ROOT_STEM[_root]
    _empty.empty_display_size = 1.5
    root_empties[_root] = _empty
bpy.context.view_layer.update()
...
#   root_empties["head"] / root_empties["tail"]  -- the two roots, each at the
#                           base of its stem, where its four ends come together
#   ROOT_MEMBERS[root]   -- the four ribbons whose ends form that root
#   ROOT_STEM[root]      -- the base of the bundle those four are pulled into
#   STEM_LEVEL[name] / STEM_AXIS[name]  -- the height one strand runs at inside
#                           that bundle and which way the bundle goes
#   TUCK_PLAN[name]      -- (root, [(bend ribbon, "under"/"over"), ...])
#   FRAMES[name]         -- (centre, forward, side) of a ribbon's own lane
...
print(
    "Step 6 ready: step 4's base model is unchanged; all eight free ribbon ends "
    f"are bent back -- {TAIL_OFFSET} cm straight past their last crossing, a "
    f"quarter turn of radius {TURN_RADIUS} cm, a {TURN_STRAIGHT} cm STRAIGHT "
    f"WALL, a second quarter turn, then the return run at {TAIL_RISE} cm above "
    "the base model.  The straight wall is now TWICE its old height, because "
    "the slot it opens has to pass TWO ends, one above the other.  "
    f"WORKED ON SO FAR: {', '.join(DOWN_BENT_RIBBONS)} -- every other ribbon is "
    "left dead straight, exactly as step 5 leaves it, so that this one can be "
    "looked at against a rim that is standing still.  Its run home ends in a "
    f"plain NINETY DEGREE BEND DOWNWARDS of radius {DOWN_RADIUS} cm -- neither "
    "turned in plan nor rolled, so it is as flat after the bend as before it -- "
    "which lands it on the WALL PLANE, the one vertical plane the straight walls "
    f"of this rim all stand in.  Then a {DROP} cm drop and a QUARTER TURN INSIDE "
    f"THAT PLANE of radius {CROSS_RADIUS} cm put it on the rim's own line, "
    f"running ACROSS green and red at {CROSS_Z} cm -- low, so that the end "
    "coming from the lane next door has the place above it.  The turn is not "
    "creased: the ribbon ROLLS a quarter turn about its own length, on over the "
    "drop and off again over the first centimetre of the rim, which is the lift "
    "a real ribbon makes when it is swung round a corner on a table.  The whole "
    "manoeuvre is over before the end reaches green.  It is still IN the wall "
    "plane, so which side of green and of red it passes is not decided yet.  "
    f"Head root: {', '.join(ROOT_MEMBERS['head'])}.  "
    f"Tail root: {', '.join(ROOT_MEMBERS['tail'])}.  "
