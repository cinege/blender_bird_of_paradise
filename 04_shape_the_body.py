...
#
# This script is self-contained: it builds the two base modules, weaves them
# tightly together and holds that finished weave STILL on frame 1.  That rest
# pose is exactly the state in which the assembly step ends -- the two modules
# are interlocked at their final registered positions and the ribbons show the
# alternating over/under pattern -- but here nothing of the assembly motion is
# keyframed any more: the modules are placed directly at their closed
# positions, so the timeline is free for the shaping animation.
#
# The shaping animation itself is NOT implemented yet; see the clearly marked
...
def add_assembly_weave_caps(rules, outline_material, parent=None):
    """Cover every module-to-module crossing on the closed weave.

    The modules no longer travel, so the caps are simply placed on the
    crossings of the finished weave -- no crossing search, no visibility or
    transform keyframes.  Parenting them keeps them on their crossing when the
    shaping stages start moving the body.
    """
    cap_length = 1.24 * W
    cap_width = 1.04 * W
    cap_lift = 0.16
    ...
    for name, top_name, under_name in rules:
        ...
        place_crossing_cap(cap, top_obj, under_obj, cap_lift)
        if parent is not None:
...
weave_caps = add_assembly_weave_caps(ASSEMBLY_WEAVE_RULES, outline_material,
                                     parent=body_root)
...
print(
    "Step 4 ready: the two modules stand woven tightly together in the closed "
    f"position (the end state of the assembly), still on frame {SHAPE_START}; "
